...
$(function () {
	...
	$("#updt").click(function(){
		...
		var f_cmt_usrid=$('select[name=f_cmt_usrid]');
		var f_cmt_atcid=$('select[name=f_cmt_atcid]');
		//先去disabled
		f_cmt_usrid.removeAttr('disabled');
		f_cmt_atcid.removeAttr('disabled');
		
		//
		if(f_cmt_usrid.val()==0){
			alert('请选择权限！');
			return false;
		}
		if(f_cmt_atcid.val()==0){
			alert('请选择模块！');
			return false;
		}
		...
		$.post(
			...
			function(data){
				//归还disabled
				f_cmt_usrid.attr('disabled','disabled');
				f_cmt_atcid.attr('disabled','disabled');
