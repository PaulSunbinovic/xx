	...
	function gtxpg(){
		...
		if($x=='vw'){
			...
			for($i=0;$i<count($xqls);$i++){
				...
				if($stdxqcls->where('f_stdxqcls_stdid='.$mo['stdid'].' AND f_stdxqcls_xqid='.$xqid)->find()&&$stdxqmj->where('f_stdxqmj_stdid='.$mo['stdid'].' AND f_stdxqmj_xqid='.$xqid)->find()){
...
// 		//给财务添加具体内容，这里以2014级为例
// 		$std=M('2014_std');
// 		$stdxqcls=M('2014_stdxqcls');
// 		$stdxqmj=M('2014_stdxqmj');
// 		$stdls=$std->join('inner join tb_2014_stdxqcls ON stdid=f_stdxqcls_stdid')->join('inner join tb_2014_stdxqmj ON stdid=f_stdxqmj_stdid')->join('tb_2014_mj ON f_stdxqmj_mjid=mjid')->where('f_stdxqcls_xqid=1 AND f_stdxqmj_xqid=1 AND f_std_statid=5')->select();
// 		$xf=M('2014_xf');$zsf=M('2014_zsf');
// 		$cw=M('2014_cw');
// 		foreach ($stdls as $v){
// 			//先确定学费
// 			$xfo=$xf->where('f_xf_bxxsid='.$v['f_mj_bxxsid'].' AND f_xf_ccid='.$v['f_mj_ccid'].' AND f_xf_klid='.$v['f_mj_klid'])->find();
// 			
// 			$dt=array(
// 				'f_cw_grdid'=>3,
// 					'f_cw_xnid'=>1,
// 					'cwzcxqid'=>1,
// 					'f_cw_stdid'=>$v['stdid'],
// 					'cwyjxfsm'=>$xfo['xfsm']+100,
// 					'cwyjjckwfsm'=>$xfo['jckwfsm'],
// 					'cwyjzsfsm'=>$zsfo['zsfsm'],
// 					'cwsjxfsm'=>$xfo['xfsm']+100,
// 					'cwsjjckwfsm'=>$xfo['jckwfsm'],
// 					'cwsjzsfsm'=>$zsfo['zsfsm'],
// 			);
// 			$cw->data($dt)->add();
// 		}
// 		p('搞定');die;
