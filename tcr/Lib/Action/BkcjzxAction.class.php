...
class BkcjzxAction extends Action{
...
// 		$ntf->setntf();
		...
		if($x=='vw'){
			...
			foreach($grdls as $v){//grdid
				...
				$pk=M($v['grdnm'].'_pk');
				$pkls=$pk->join('tb_'.$v['grdnm'].'_kc ON f_pk_kcid=kcid')->where('f_pk_grdid='.$v['grdid'].' AND f_pk_sttid='.$tcro['f_tcr_sttid'].' AND f_pk_xqid='.$xqid.' AND f_pk_tcrid='.$tcro['tcrid'])->select();
				...
				foreach($pkls as $u){
					...
					$cjzxls=$cjzx->Distinct(true)->field('clsid,clsnm,cjzxsftj')->where('f_stdxqcls_xqid='.$xqid.' AND f_stdxqmj_xqid='.$xqid.' AND f_cjzx_pkid='.$u['pkid'].' AND f_std_statid=5')->select();
					
					//其实算是获得了班级，那么就看看，这个班级，这个排课，是否大家都过了
					$clslsfn=array();
					foreach ($cjzxls as $clsv) {
						# code...
						//看下有没有需要补考的学生，没的话就不用体现出来了
						$cjzx=M($v['grdnm'].'_cjzx');
						$cjzxnb=$cjzx->join('inner join tb_'.$v['grdnm'].'_stdxqcls ON f_cjzx_stdid=f_stdxqcls_stdid')->where('f_cjzx_sttid='.$sttid.' AND f_cjzx_xqid='.$xqid.' AND f_stdxqcls_xqid='.$xqid.' AND f_cjzx_pkid='.$u['pkid'].' AND f_stdxqcls_clsid='.$clsv['clsid'].' AND (cjzxhk=1 OR(cjzxzf>30 AND cjzxzf<60))')->count();

						$cjzxbkfnb=$cjzx->join('inner join tb_'.$v['grdnm'].'_stdxqcls ON f_cjzx_stdid=f_stdxqcls_stdid')->where('f_cjzx_sttid='.$sttid.' AND f_cjzx_xqid='.$xqid.' AND f_stdxqcls_xqid='.$xqid.' AND f_cjzx_pkid='.$u['pkid'].' AND f_stdxqcls_clsid='.$clsv['clsid']." AND (cjzxhk=1 OR(cjzxzf>30 AND cjzxzf<60)) AND (cjzxbkf='' OR cjzxbkf IS NULL)")->count();
						if($cjzxbkfnb>0){
							//说明未提交
							$clsv['bktjmk']=0;
						}else{
...
// 		$cjzxls=$cjzx->select();
		...
		$spccdtint='-sp-(cjzxhk=1 OR (cjzxzf>30 AND cjzxzf<60))-sp-';////
		...
		//判断是否这个班级的补考分都登过了，也就是所谓的提交过了
		$cjzxforbkfnb=clone M($grdo['grdnm'].'_cjzx');
		$cjzxbkfnb=$cjzxforbkfnb->join('inner join tb_'.$grdo['grdnm'].'_stdxqcls ON f_cjzx_stdid=f_stdxqcls_stdid')->where('f_cjzx_sttid='.$sttid.' AND f_cjzx_xqid='.$xqid.' AND f_stdxqcls_xqid='.$xqid.' AND f_cjzx_pkid='.$pkid.' AND f_stdxqcls_clsid='.$clsid." AND (cjzxhk=1 OR(cjzxzf>30 AND cjzxzf<60)) AND (cjzxbkf='' OR cjzxbkf IS NULL)")->count();
		if($cjzxbkfnb>0){
			$this->assign('zttjbkf',0);//总体提交补考分
		}else{
			$this->assign('zttjbkf',1);
		}
				

		for($i=0;$i<count($mls);$i++){
			if($cjzxbkfnb>0){$mls[$i]['bktjmk']=0;}else{$mls[$i]['bktjmk']=1;}
		...
		//q特殊
		$this->assign('title','补考成绩中心');
		$this->assign('theme','补考成绩录入');
