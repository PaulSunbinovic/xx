...
$.fn.smartFloatSR = function() {
	var position = function(element) {
		...
		$(window).scroll(function() {
			...
			if (scrolls > 600) {//且在可视范围内//屏幕太小表格不会很大而且触发了responsive，所以不执行
				if (window.XMLHttpRequest) {
					
					element.css({
						
						position: "fixed",
						top: $(window).height()-$('.share-group').height()-8
					});	
				} else {
...
//share专用
$.fn.smartFloatQR = function() {
	var position = function(element) {
		var top = element.position().top, pos = element.css("position");
		var zldtp=$('#zld').offset().top;
		$(window).scroll(function() {
			var scrolls = $(this).scrollTop()+$('header').height();//定位在低点
			if ($(window).width()>600&&scrolls>(zldtp)&&scrolls<1800) {//且在可视范围内//屏幕太小表格不会很大而且触发了responsive，所以不执行
				if (window.XMLHttpRequest) {
...
//BD专用
$.fn.smartFloatBD = function() {
	var position = function(element) {
		var top = element.position().top, pos = element.css("position");
		var zldtp=$('#zld').offset().top;
		$(window).scroll(function() {
			var scrolls = $(this).scrollTop();//定位在最顶上
			if (scrolls>(zldtp)) {//且在可视范围内//屏幕太小表格不会很大而且触发了responsive，所以不执行
				if (window.XMLHttpRequest) {
