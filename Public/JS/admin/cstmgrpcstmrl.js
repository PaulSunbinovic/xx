...
$(function () {
	...
	$("#updt").click(function(){
		...
		var f_cstmgrpcstmrl_cstmgrpid=$('select[name=f_cstmgrpcstmrl_cstmgrpid]');
		var f_cstmgrpcstmrl_cstmrlid=$('select[name=f_cstmgrpcstmrl_cstmrlid]');
//		//先去disabled
//		f_cstmgrpcstmrl_usrid.removeAttr('disabled');
//		f_cstmgrpcstmrl_cstmrlid.removeAttr('disabled');
		
		//
		if(f_cstmgrpcstmrl_cstmgrpid.val()==0){
			alert('请选择团队！');
			return false;
		}
		if(f_cstmgrpcstmrl_cstmrlid.val()==0){
			alert('请选择角色！');
			return false;
		}
		...
		$.post(
			...
			function(data){
//				//归还disabled
//				f_cstmgrpcstmrl_usrid.attr('disabled','disabled');
//				f_cstmgrpcstmrl_cstmrlid.attr('disabled','disabled');
...
function dlt(cstmgrpcstmrlid){
	if(confirm("确定要删除此条记录？")){
		$.post(
			hdldlt,
			{cstmgrpcstmrlid:cstmgrpcstmrlid},
			function(data){
