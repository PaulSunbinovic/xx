...
$(function () {
	...
	$("#updt").click(function(){
		...
		var f_cstmcmt_cstmusrid=$('select[name=f_cstmcmt_cstmusrid]');
		var f_cstmcmt_atcid=$('select[name=f_cstmcmt_atcid]');
		//先去disabled
		f_cstmcmt_cstmusrid.removeAttr('disabled');
		f_cstmcmt_atcid.removeAttr('disabled');
		
		//
		if(f_cstmcmt_cstmusrid.val()==0){
			alert('请选择权限！');
			return false;
		}
		if(f_cstmcmt_atcid.val()==0){
			alert('请选择模块！');
			return false;
		}
		...
		$.post(
			...
			function(data){
				//归还disabled
				f_cstmcmt_cstmusrid.attr('disabled','disabled');
				f_cstmcmt_atcid.attr('disabled','disabled');
