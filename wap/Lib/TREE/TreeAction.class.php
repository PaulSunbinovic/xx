...
// 		}
		...
		//BD组合多维数组结果以List形式（ul li）
		Static Public function unlimitedForListNv($cate,$pid,$idzd,$nmzd,$pidzd,$odrzd){
			$str='';
			foreach ($cate as $v) {
				...
				if ($v[$pidzd] == $pid) {
					...
					$rslt=self::unlimitedForListNv($cate,$v[$idzd],$idzd,$nmzd,$pidzd,$odrzd);
					if($rslt==''){//无子嗣就算了
						$str=$str."<li>"."<a href='__APP__/Atc/query/bdid/".$v['bdid']."'>".$v[$nmzd]."</a>";
					}else{
						$str=$str."<li>"."<a href='__APP__/Atc/query/bdid/".$v['bdid']."'>".$v[$nmzd]."</a>".$rslt;
					}
					$str=$str.'</li>';
					//$arr[] = $v;
				}
...
// 			foreach ($cate as $v){
// 				if($v[$idzd]==$id){
// 					$str=self::findF($cate,$v[$pidzd],$idzd,$nmzd,$pidzd).'&gt;&gt;'.$v[$nmzd];
// 					break;
// 				}
// 			}
// 			return $str;
// 		}老方法
		...
		Static Public function findF($cate,$id,$idzd,$nmzd,$pidzd){
			...
			foreach ($cate as $v){
				if($v[$idzd]==$id){
					$str=self::findF($cate,$v[$pidzd],$idzd,$nmzd,$pidzd).'<li><a href="'.__APP__.'/Atc/query/bdid/'.$id.'">'.$v[$nmzd].'</a> </li>';
					break;
				}
			}
			return $str;
		...
		Static Public function findFa($cate,$id,$idzd,$nmzd,$pidzd){
			$str='';
			foreach ($cate as $v){
				if($v[$idzd]==$id){
					$str=self::findF($cate,$v[$pidzd],$idzd,$nmzd,$pidzd).'&gt;&gt;'."<a href='__APP__/Atc/query/bdid/".$v[$idzd]."'>".$v[$nmzd]."</a>";
					break;
				}
			}
			return $str;
