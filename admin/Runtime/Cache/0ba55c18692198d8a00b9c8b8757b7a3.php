...
<table>
<tr><td>用户名：</td><td><input type='text' name='unm' value="<?php echo ($mo['unm']); ?>" id='unm'/></td></tr>
<tr><td>密码：</td><td><input type='text' name='upw' value="<?php echo ($mo['upw']); ?>" id='upw'/></td></tr>
<tr><td>再输入一次密码：</td><td><input type='text' name='upwag' value="<?php echo ($mo['upw']); ?>" id='upwag'/></td></tr>
<tr><td>用户头像：</td><td>
...
<div id="imgg">
<img src="<?php echo ($mo['upt']); ?>" alt="" style="width:100px" />
</div>
<div style="width:80px; height:26px; overflow:hidden;"><input type="file"  name="photo1" id="uploadify"/></div>
<!-- 隐藏系统所需的用户upt -->
<input type='hidden' name='upt' value="<?php echo ($mo['upt']); ?>" id='pt'/>
<!-- file over -->
...
<script>$('select[name=f_athid]').val(<?php echo ($mo['f_athid']); ?>)</script>
</td>
</tr>

</table>

<input type='button' id='updt' value=<?php echo ($btnvl); ?> />
</form>
</div>
