...
$.fn.smartFloatSR = function() {
	var position = function(element) {
		...
		$(window).scroll(function() {
			...
			if ($(window).width()>600&&scrolls > 600&&scrolls<=($('#zld').offset().top+element.outerHeight())) {//且在可视范围内//屏幕太小表格不会很大而且触发了responsive，所以不执行
				if (window.XMLHttpRequest) {
...
$.fn.smartFloatQR = function() {
	var position = function(element) {
		...
		$(window).scroll(function() {
			...
			if ($(window).width()>600&&scrolls>(zldtp)) {//且在可视范围内//屏幕太小表格不会很大而且触发了responsive，所以不执行
				if (window.XMLHttpRequest) {
