...
$(function () {
	...
	$("#updt").click(function(){
		...
		var f_lbmd_lbid=$('select[name=f_lbmd_lbid]');
		var f_lbmd_mdid=$('select[name=f_lbmd_mdid]');
//		//先去disabled
//		f_lbmd_lbid.removeAttr('disabled');
//		f_lbmd_mdid.removeAttr('disabled');
		
		//
		if(f_lbmd_lbid.val()==0){
			alert('请选择类别！');
			return false;
		}
		if(f_lbmd_mdid.val()==0){
			alert('请选择模块！');
			return false;
		}
		...
		$.post(
			...
			function(data){
//				//归还disabled
//				f_lbmd_lbid.attr('disabled','disabled');
//				f_lbmd_mdid.attr('disabled','disabled');
